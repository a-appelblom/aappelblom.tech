...
const REDEPLOY_URL = import.meta.env.VITE__VERCEL_REDEPLOY_URL;
...
export async function updateInstagramToken() {
	...
	const expiry = data.envs.find((env) => env.key === 'VITE_INSTAGRAM_EXPIRY_DATE');
	...
	if (accessToken && expiry) {
		...
		// Trigger redeploy
		if (accessToken.status === 200 && expiry.status === 200) {
			console.log('___________________________________________________________');
